...
from Pyrochiy import BOTLOG_CHATID, LOGGER
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Pyrochiy-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**Pyrochiy-Userbot Berhasil di matikan!**")
    if HAPP is not None:
...
async def logs_ubot(client: Client, message: Message):
    ...
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="Pyrochiy/resources/logo.jpg",
