...
from Pyrochiy import app, CMD_HELP
from Pyrochiy.helpers.cmd import *
from Pyrochiy.helpers.PyroHelpers import ReplyCheck
from Pyrochiy.helpers.utility import split_list
...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        try:
            ...
            ac = PrettyTable()
            ac.header = False
            ac.title = "Pyrochiy-UserBot Plugins"
            ac.align = "l"
            for x in split_list(sorted(CMD_HELP.keys()), 2):
                ac.add_row([x[0], x[1] if len(x) >= 2 else None])
            xx = await client.send_message(
                message.chat.id,
                f"```{str(ac)}```\n• @ShicyyXCode × @ShicyxC0d •",
